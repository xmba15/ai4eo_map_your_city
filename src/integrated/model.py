...
__all__ = (
    "MultiModalNetPl",
    "MultiModalNetSharedStreetOrthoPl",
        ...
        self.losses = [
            (
                ...
                get_object_from_dict(
                    self.hparams["loss"]["classification"],
                ),
            ),
...
class MultiModalNetSharedStreetOrthoPl(pl.LightningModule):
    def __init__(self, hparams):
        super().__init__()
        self.hparams.update(hparams)

        self.model = MultiModalNetSharedStreetOrtho(
            self.hparams["model"]["encoder_name"],
    ...
    def common_step(self, batch, batch_idx, is_val: bool = False):
        ...
        for loss_name, weight, loss_class in self.losses:
            if loss_name == "focal":
                ...
                assert loss_name == "distribution"
                cur_loss = loss_class(shared_feats[0], shared_feats[1])
...
class MultiModalNetFullModalityPl(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.accuracy = Accuracy(task="multiclass", num_classes=self.hparams["model"]["num_classes"])
        self.loss = get_object_from_dict(
            self.hparams["loss"]["classification"],
        )
...
class MultiModalNetFullModalityAttentionFusionPl(pl.LightningModule):
    def __init__(self, hparams):
        super().__init__()
        self.hparams.update(hparams)
    ...
    def common_step(self, batch, batch_idx, is_val: bool = False):
        images, s2_data, country_id, label = batch
        logits, ortho_feat, street_feat, s2_feat = self.forward(images, s2_data, country_id)
        batch_size = logits.shape[0]
